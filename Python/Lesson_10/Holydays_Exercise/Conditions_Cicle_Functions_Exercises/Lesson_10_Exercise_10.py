...
memorizza_file([1100, 20000, 1048576, 512, 5000]) -->   File di 1100 byte compresso in 880.0 byte e memorizzato. Blocchi usati: 2. Blocchi rimanenti: 998.
...
def memorizza_file(files: list[int]) -> None:
    ...
    for file in files:
        ...
        filecomp = file*0.8
        used_blocks = round(filecomp/512)

        if used_blocks < spazio_totale_blocchi:
            spazio_totale_blocchi = spazio_totale_blocchi - used_blocks
            print(f"File di {file} byte compresso in {filecomp} byte e memorizzato. Blocchi usati: {used_blocks}. Blocchi rimanenti: {spazio_totale_blocchi}.")
